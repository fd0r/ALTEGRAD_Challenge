...
class DeepWalk():
    ...
    def __init__(self, walk_length=80, n_walks=50, p=0, n_features=128, window=8, epochs=5, training_method=1, hs=1, negative=0, workers=8, load_path=None, verbose=True):
    ...
    def random_walk(self, G, node):
        ...
        while len(neighs) > 0 and t < self.walk_length:
            if np.random.rand() < self.p:
                weights = np.array([node_data['weight'] for  node_data in dict(G[node]).values()])
